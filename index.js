...
function viewResult(){
    ...
    if(result.status === 'success'){
        document.getElementById('result').innerHTML += `<br><br><br><h1 style="color:greenyellow;font-size:60px;font-weight:bolder">!!<em>${result.status}</em>!!</h1>
                                                        <p style="font-size:30px;font-weight:bold">Found Falcone!! Congratulations! for Finding Falcone. King Shan is mighty pleased.</p>
                                                        <div style="display:inline-block;border-radius:50%;background-color:rgba(250, 235, 215, 0.7);padding:30px;color:blue;"><div>Planet : ${result.planet_name}</div>
                                                        <div>Time taken: ${time}</div>
                                                        <div>Best Time : ${localStorage.getItem('timeBest')}</div></div>
                                                        `;
    }

    else{
        document.getElementById('result').innerHTML += '<h1 style="color:red;font-size:60px;font-weight:bolder">Mission <em>Failed</em>!!</h1>';
    }
